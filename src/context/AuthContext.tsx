...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async ({ username, password }: { username: string; password: string }) => {
    ...
    setUser({ id: data.userId, username: data.username, displayName: data.displayName, role: data.role });
    setToken(data.token);
    localStorage.setItem('yomu_user', JSON.stringify({ id: data.userId, username: data.username, displayName: data.displayName, role: data.role }));
